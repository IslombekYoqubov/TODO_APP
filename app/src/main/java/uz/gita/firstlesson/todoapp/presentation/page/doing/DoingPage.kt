...
import uz.gita.firstlesson.todoapp.databinding.PageDoingBinding
...
import uz.gita.firstlesson.todoapp.presentation.factories.DoingViewModelFactory
...
class DoingPage : Fragment(R.layout.page_doing) {
    ...
    private val viewModel: DoingViewModel by viewModels {
        DoingViewModelFactory(AppRepositoryImpl.getInstance())
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val itemTouchHelper = ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.RIGHT) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                viewModel.changeTaskStatus(task.id, 2)
            }
        })

        val itemTouchHelper1 = ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            override fun onMove(
                recyclerView: RecyclerView,
                viewHolder: RecyclerView.ViewHolder,
                target: RecyclerView.ViewHolder
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                val task = adapter.currentList[viewHolder.absoluteAdapterPosition]
                viewModel.changeTaskStatus(task.id, 0)
            }
        })

        itemTouchHelper.attachToRecyclerView(binding.list)
        itemTouchHelper1.attachToRecyclerView(binding.list)

        adapter.setSelectItemListener {
        ...
        viewModel.taskEntityLiveData.observe(viewLifecycleOwner, taskEntityLiveDataObserver)
    ...
    private val taskEntityLiveDataObserver = Observer<List<TaskEntity>> {
